...
import { LoansAndAdvancesScheduleData, LoanAdvanceItem, ScheduleData } from '../../types.ts';
...
export const LoansAndAdvancesSchedule: React.FC<LoansAndAdvancesScheduleProps> = ({ data, onUpdate, isFinalized }) => {
    ...
    const addItem = () => {
        const newRow: LoanAdvanceItem = { id: uuidv4(), particular: '', amountCy: '', amountPy: '' };
            ...
            <div className="space-y-2">
                {data.items.map(item => (
                    <div key={item.id} className="flex items-center space-x-2 bg-gray-900/50 p-2 rounded-lg">
                        ...
                        <input type="text" placeholder="Amount CY" value={item.amountCy} onChange={e => handleItemUpdate(item.id, 'amountCy', e.target.value)} disabled={isFinalized} className="w-1/4 bg-gray-700 p-2 rounded-md"/>
                        <input type="text" placeholder="Amount PY" value={item.amountPy} onChange={e => handleItemUpdate(item.id, 'amountPy', e.target.value)} disabled={isFinalized} className="w-1/4 bg-gray-700 p-2 rounded-md"/>
